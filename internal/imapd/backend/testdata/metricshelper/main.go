// Command metricshelper is a stand-in protocol-handler used by the imapd
// dispatcher metrics end-to-end test. The real handler needs a running
// session-manager to serve an IMAP session, so this helper skips the session
...
// the report to the parent over the fd-4 pipe, exactly as cmd/imapd/handler.go
// does at session end.
//
...
func main() {
	...
	c.CommandProcessed("SELECT")
	c.AuthAttempt("example.com", true)
	c.MessageFetched("example.com", 1234)
	c.ConnectionClosed()
