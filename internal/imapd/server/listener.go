...
// It receives the context and connection, and should handle the IMAP session.
type ConnectionHandler func(ctx context.Context, conn *Connection)

// Listener manages a single TCP listener for accepting IMAP connections.
...
func (l *Listener) Start(ctx context.Context) error {
	...
	// For IMAPS mode, wrap with TLS immediately
	if l.mode == config.ModeImaps {
		if l.tlsConfig == nil {
			return errors.New("TLS configuration required for IMAPS mode")
		}
...
func (l *Listener) handleConnection(ctx context.Context, netConn net.Conn) {
	...
	if l.limiter != nil && !l.limiter.TryAcquire() {
		...
		_, _ = netConn.Write([]byte("* BYE Server busy, try again later\r\n"))
		_ = netConn.Close()
		return
	}
	if l.limiter != nil {
...
// For non-IMAPS modes, this can be used for STARTTLS.
