...
// Server coordinates multiple listeners and handles IMAP connections.
type Server struct {
	cfg       *config.Config
...
func (s *Server) Run(ctx context.Context) error {
	...
	for _, lc := range s.cfg.Listeners {
		...
		if lc.Mode == config.ModeImaps {
			if s.tlsConfig == nil {
				s.mu.Unlock()
				return fmt.Errorf("listener %s: TLS required for IMAPS mode but not configured", lc.Address)
			}
			tlsCfg = s.tlsConfig
		} else if s.tlsConfig != nil {
			// Make TLS available for STARTTLS on non-IMAPS listeners
			tlsCfg = s.tlsConfig
		}
...
// This should be replaced with actual IMAP protocol handling.
