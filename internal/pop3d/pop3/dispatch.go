...
const (
	EnvClientIP     = "POP3D_CLIENT_IP"
	EnvListenerMode = "POP3D_LISTENER_MODE"
)

// DispatcherConfig configures the pop3d listener process, the parent half of
// the fork-per-connection model (mail-security-model.md, #179).
type DispatcherConfig struct {
	Config config.Config
	// ExecPath is the pop3d binary handlers are spawned from
...
// subprocess per connection. It never speaks POP3 and never holds session
// state.
...
// NewDispatcher validates cfg and builds the dispatcher. pop3s listeners
...
func NewDispatcher(cfg DispatcherConfig) (*Dispatcher, error) {
	...
	for _, lc := range cfg.Config.Listeners {
		if lc.Mode == config.ModePop3s && tlsCert == "" {
			return nil, fmt.Errorf("listener %s: pop3s requires tls cert_file and key_file", lc.Address)
		}
	}
	...
	for _, lc := range cfg.Config.Listeners {
		// Every pop3d listener is auth-facing: authentication is the point of
		// the protocol on both 110 and 995 (#225).
...
// the handler's config re-read (pop3d has no env overlay; the flag plumbing
