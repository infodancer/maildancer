...
func TestRunSingleConn_SessionEndsAfterQuit(t *testing.T) {
	...
	go func() {
		stack.RunSingleConn(serverConn, config.ModePop3) //nolint:errcheck // test connection torn down by the pipe closing; the resulting error is expected and not checked
		close(done)
	}()
